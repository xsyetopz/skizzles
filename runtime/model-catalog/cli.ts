...
function recordSwitch(found: Set<string>, flag: string): void {
  if (found.has(flag)) throw new Error(`${flag} must not be repeated`);
  found.add(flag);
}
...
function parseOptions(
  ...
  const allowedValues = new Set<string>(valueFlags);
...
function required(options: ParsedOptions, flag: ValueFlag): string {
  ...
  if (found === undefined) throw new Error(`${flag} is required`);
  return found;
}
...
export function runModelCatalogCli(args: string[]): Promise<void> {
  ...
  if (command === "refresh") return runRefresh(options, false);
  if (command === "service") return runRefresh(options, true);
