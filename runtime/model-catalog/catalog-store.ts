...
import { assertCompleteCatalog, type JsonObject } from "./catalog-schema.ts";
...
async function firstMissingComponent(
  ...
  for (const component of pathComponents(path)) {
    if ((await existingPathMetadata(component)) === undefined) return component;
  }
  return undefined;
}
...
async function validatePrivateDirectoryChain(
  ...
  const expectedUid = globalThis.process.getuid?.();
  for (const component of pathComponents(directory)) {
    if (!within(privacyRoot, component)) continue;
...
async function privacyRootFor(
  ...
  if (within(codexHome, directory)) return codexHome;
...
async function ensurePrivateFileMode(path: string): Promise<void> {
  ...
  if ((regular.mode & 0o777) === 0o600) return;
...
async function validateCatalogTarget(
  ...
  if (inspection.metadata === undefined) return inspection;
  const expectedUid = globalThis.process.getuid?.();
  if (
...
export async function readBoundedJsonFile(
  ...
  try {
    ...
    return JSON.parse(contents.subarray(0, length).toString("utf8"));
  } finally {
    await handle.close();
  }
}
...
export async function cachedCatalog(
  ...
  try {
    const value = await readBoundedJsonFile(path, maxBytes);
...
async function preparedTarget(
  ...
  if (expected.target === undefined) return { expected, unchanged: false };
...
export async function writePrivateAtomic(
  ...
  if (target.unchanged) return false;
