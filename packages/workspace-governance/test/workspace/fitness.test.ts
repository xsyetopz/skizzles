...
describe("mandatory workspace architecture fitness", () => {
  ...
  it("rejects directories with more than ten authored files", async () => {
    const root = await fixture();
    const crowded = join(root, "packages/example/src/flat");
    await mkdir(crowded, { recursive: true });
    await Promise.all(
      Array.from({ length: 11 }, (_, index) =>
        writeFile(join(crowded, `part-${index}.ts`), "export {};\n"),
      ),
    );

    expect(await validateWorkspace(root)).toContainEqual({
      code: "directory-file-limit",
      path: "packages/example/src/flat",
      message: "directory contains 11 authored files; maximum is 10",
    });
  });
    ...
    expect(findings).toEqual([
      {
        ...
        message:
          "hard-coded-host-temp is disposable temporary-root authority owned by @skizzles/scratchspace",
      },
      {
        code: "disposable-temp-ownership",
        path: "packages/example/src/index.ts",
        message:
          "mkdtemp is disposable temporary-root authority owned by @skizzles/scratchspace",
      },
      {
        code: "disposable-temp-ownership",
        path: "packages/example/src/index.ts",
        message:
          "tmpdir is disposable temporary-root authority owned by @skizzles/scratchspace",
      },
    ]);
  });
