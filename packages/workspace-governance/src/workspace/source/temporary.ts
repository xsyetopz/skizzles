...
const TEMPORARY_OWNERSHIP_DISPOSITIONS: readonly TemporaryOwnershipDisposition[] =
  [
    disposition(
      packagePath("scratchspace", "src/process/platform.ts"),
      ["mkdtemp", "tmpdir"],
      "@skizzles/scratchspace is the sole disposable run-root platform authority.",
    ),
    disposition(
    ...
    disposition(
      packagePath("command-observation", "src/codex-command/settings.ts"),
      ["tmpdir"],
  ...
  for (const { kind } of uses) {
    if (!exactDisposition?.allowedUses.includes(kind)) {
      addFinding(
        ...
        path,
        `${kind} is disposable temporary-root authority owned by @skizzles/scratchspace`,
      );
    }
  }
}
