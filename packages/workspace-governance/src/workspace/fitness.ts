...
import { validatePackageDirectoryFileCounts } from "./layout.ts";
...
const ARTIFACT_COMPOSITION_OWNERS = new Set(["@skizzles/plugin-packaging"]);
...
export async function validateWorkspaceFitness(
  ...
  for (const item of packages) {
    await validatePackageDirectoryFileCounts(item, findings);
