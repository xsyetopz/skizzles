...
export function parseBaselineFile(value: unknown): BaselineFile {
  ...
  if (object.version !== 1) invalid("synchronization baseline version");
...
export function parseStoredPreview(value: unknown): StoredPreview {
  ...
  if (object.version !== 1) invalid("synchronization preview version");
...
export function parseSyncJournal(value: unknown): SyncJournal {
  ...
  if (object.version !== 1) invalid("synchronization journal version");
  if (
  ...
  for (const mutated of mutatedPaths) {
    if (!paths.includes(mutated)) invalid("journal mutated path provenance");
  }
  ...
  for (const backup of backups) {
    ...
    if (intended === undefined) invalid("journal applied state coverage");
    if (!backup.publication) invalid("journal publication provenance");
  }
  return {
    version: 1,
    state: object.state,
...
function parseChanges(value: unknown): SyncChange[] {
  if (!Array.isArray(value)) invalid("preview changes");
...
function parseConflicts(value: unknown): SyncConflict[] {
  if (!Array.isArray(value)) invalid("preview conflicts");
...
function parseBackups(value: unknown): BackupRecord[] {
  if (!Array.isArray(value)) invalid("journal backups");
  const backups = value.map((entry, index) => {
    ...
    if (!existed) {
      if (object.original !== null) invalid(`journal backup ${index} original`);
...
function parseSyncFile(
  ...
  if (object.path !== relative) invalid(`${label} path`);
  const kind = object.kind;
  if (kind !== "file" && kind !== "symlink") invalid(`${label} kind`);
...
function parseDirection(value: unknown): "push" | "pull" {
  if (value !== "push" && value !== "pull") invalid("preview direction");
  return value;
}
...
function stringMatching(
  ...
  if (!pattern.test(string)) invalid(label);
  return string;
}
