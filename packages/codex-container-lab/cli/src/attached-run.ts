...
import type { StateRoots } from "./state.ts";
import { activityLockPath, labLockPath, readLab, writeLab } from "./state.ts";
...
export async function runAttachedCommand(
  ...
  try {
    ...
    if (signal?.aborted) return abortExitCode(signal);
    throw error;
  }
}
...
function abortExitCode(signal: AbortSignal | undefined): number {
