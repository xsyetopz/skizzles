...
import type { StoredPreview, SyncJournal } from "./sync-contract.ts";
import { previewBinding } from "./sync-preview.ts";
...
async function crashApply(
  ...
  const modulePath = path.join(import.meta.dir, "sync-apply.ts");
  const script = `
...
async function replaceNestedParent(
  ...
  if (replacement === "recreated") await mkdir(parent);
...
describe("guarded preview and apply", () => {
  ...
  test("strictly rejects unknown and duplicate stored preview fields", async () => {
    for (const tamper of ["unknown", "duplicate"] as const) {
      ...
      if (tamper === "unknown") stored.unexpected = true;
      else {
        stored.changes.push(
  ...
  test("preserves a target directory created during the final idle guard", async () => {
    ...
    await expect(
      applySync({
        ...
        idleGuard: async () => {
          guards++;
          if (guards === 2) await mkdir(path.join(state.target, "nested"));
        },
      }),
