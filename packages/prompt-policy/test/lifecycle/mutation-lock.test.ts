...
describe("exclusive prompt mutation lock contracts", () => {
  ...
  test("refuses a live external lock owner", async () => {
    ...
    try {
      ...
      child.kill();
      await child.exited;
      await rm(join(root, "packages/prompt-policy/assets/.mutation-lock"), {
        force: true,
        recursive: true,
      });
    }
  ...
  test("reclaims a stale owner and preserves a concurrent replacement lock", async () => {
    ...
    await writeFile(
      join(root, "packages/prompt-policy/assets/.mutation-lock/reclaim.json"),
      `${JSON.stringify(
        {
          version: 1,
          pid: stalePid,
    ...
    expect(
      await Bun.file(
        join(root, "packages/prompt-policy/assets/.mutation-lock"),
      ).exists(),
    ).toBe(false);
    ...
    await rm(join(racedRoot, "packages/prompt-policy/assets/.mutation-lock"), {
      force: true,
      recursive: true,
    });
  });
  ...
  test("never removes a replacement lock when a delayed initializer resumes", async () => {
    ...
    expect(
      await Bun.file(
        join(root, "packages/prompt-policy/assets/.mutation-lock/owner.json"),
      ).exists(),
    ).toBe(true);
  ...
  test("handles incomplete-lock clock skew without deleting a live replacement", async () => {
    ...
    const futureLock = join(
      futureRoot,
      "packages/prompt-policy/assets/.mutation-lock",
    );
  ...
  test("recovers proven-dead stale and release quarantines but preserves malformed remnants", async () => {
    ...
    const staleOrphan = join(
      staleRoot,
      `packages/prompt-policy/assets/.mutation-stale-${staleToken}`,
    );
    ...
    expect(
      (await readdir(join(releaseRoot, "packages/prompt-policy/assets"))).some(
    ...
    const malformed = join(
      malformedRoot,
      "packages/prompt-policy/assets/.mutation-stale-not-a-token",
    );
    ...
    const liveOrphan = join(
      liveRoot,
      `packages/prompt-policy/assets/.mutation-release-${liveToken}`,
    );
    ...
    const unrelated = join(
      unrelatedRoot,
      "packages/prompt-policy/assets/.mutation-unrelated",
    );
