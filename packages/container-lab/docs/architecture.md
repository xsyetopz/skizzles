...
The Compose pipeline first reads only source/base files with `--env-file /dev/null` and `config --no-interpolate --no-normalize --no-env-resolution --format json`. That raw model expands Compose merge, `include`, and `extends` semantics, preserves valueless service environment and build arguments, recognizes braced, unbraced, default, alternative, required, and nested interpolation while retaining `$$` literals, and rejects undeclared reads, project `.env`, service `env_file`, or `configs.environment`. The exact validated JSON is materialized as private `source.compose.json`; the generated override and every normalized validation, up, status, logs, attached execution, and termination operation use only that bound source. Original Compose-file mutation therefore cannot change a prepared or ready lab. Docker subprocesses receive an exact client environment plus only the present manifest-authorized non-secret names; ambient `COMPOSE_*` topology controls are absent. The service-owned invoking environment is threaded through every Docker path rather than reread globally.

Git repository discovery, identity checks, local no-hardlinks cloning, remote removal, and detached checkout share one process adapter. It supplies only path/temp/locale plus fixed no-system/no-global/no-prompt settings and command-scoped disabled hooks and filesystem monitors. Ambient Git repository, object, index, credential, SSH, trace, pager, and editor controls are absent. This is process-boundary isolation for trusted local repositories, not a hostile-source sandbox.
