...
describe("guarded preview and apply", () => {
  it("applies content and mode, persists details, calls the idle guard, and consumes the token", async () => {
  ...
  it("previews and applies prototype-shaped tracked paths as ordinary records", async () => {
    const state = await fixture();
  ...
  it("applies deletions and symlinks without following them", async () => {
    const state = await fixture();
  ...
  it("rejects a target parent changed into a symlink after preview", async () => {
    const state = await fixture();
    for (const root of [state.source, state.target]) {
  ...
  it("rejects stale, expired, direction-mismatched, lab-mismatched, and busy applies", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("does not issue an apply token for a preview too large to expose", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    await expect(
      previewSync({
  ...
  it("rejects a target edit racing with staging and preserves it", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("runs the final idle guard before entry validation and preserves a racing target edit", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("rejects a rebound injected upsert before target mutation", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("strictly rejects unknown and duplicate stored preview fields", async () => {
  ...
  it("rejects a symlink-swapped stored preview before mutation", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("consumes a token but removes all preparation artifacts after a source race", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("does not claim a token when the first idle guard refuses apply", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "source\n");
    const base = {
      ...state,
  ...
  it("preserves a target directory created during the final idle guard", async () => {
    const state = await fixture();
