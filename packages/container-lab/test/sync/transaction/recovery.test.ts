...
describe("transaction recovery", () => {
  it("rolls back a published deletion from its durable backup", async () => {
  ...
  for (const replacement of ["removed", "recreated", "symlink"] as const) {
    it(`retains a nested deletion journal when its parent is ${replacement}`, async () => {
      const state = await fixture();
      for (const root of [state.source, state.target]) {
        await mkdir(path.join(root, "nested"));
        await writeFile(path.join(root, "nested", "delete.txt"), "base\n");
  ...
  it("removes journal-owned parent directories after a pre-rename crash", async () => {
    const state = await fixture();
  ...
  it("retains a pre-publication journal when a created parent is replaced", async () => {
    const state = await fixture();
    await mkdir(path.join(state.source, "nested", "deep"), { recursive: true });
    await writeFile(
      path.join(state.source, "nested", "deep", "new.txt"),
  ...
  it("retains an unverified directory created before its identity was journaled", async () => {
    const state = await fixture();
  ...
  it("removes a crash-created publication artifact while retaining the original target", async () => {
    const state = await fixture();
  ...
  it("rolls back an atomically published target using the production journal descriptor", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
    const crash = await crashApply(state);
  ...
  it("finishes baseline publication after all targets were durably applied", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
  ...
  it("refuses baseline publication after an applied-state user divergence", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
    const crash = await crashApply(state, "after-journal");
    const syncRoot = path.join(state.stateRoot, "sync", state.labId);
  ...
  it("treats a durable baseline as committed and preserves later target edits", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
  ...
  it("preserves a divergent edit made after publication and retains recovery state", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
    const crash = await crashApply(state);
  ...
  it("rejects journal roots and baseline paths outside the lab allowlist", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
    const crash = await crashApply(state);
    const syncRoot = path.join(state.stateRoot, "sync", state.labId);
  ...
  for (const entry of malformedJournalCases) {
    it(`rejects malformed journal ${entry.name} before recovery writes`, async () => {
      const state = await fixture();
      await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
      const crash = await crashApply(state);
  ...
  it("rejects swapped production backup records before recovery writes", async () => {
    const state = await fixture();
    for (const root of [state.source, state.target]) {
  ...
  it("rejects a symlink-swapped backup directory before target writes", async () => {
    const state = await fixture();
    await writeFile(path.join(state.source, "file.txt"), "plugin write\n");
    const crash = await crashApply(state);
