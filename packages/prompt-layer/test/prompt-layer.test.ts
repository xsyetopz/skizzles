...
describe("pinned Codex prompt layer", () => {
  ...
  test("requires immutable rebase refs and explicit reviewed replay", async () => {
    ...
    const fetcher = async (url: string) => {
      ...
      return {
        status: 200,
        body: await readFile(
          join(root, "packages/prompt-layer/assets/upstream/default.md"),
        ),
      };
    };
    const before = await snapshot(root);
    ...
    const manifest = (await Bun.file(
      join(root, "packages/prompt-layer/assets/manifest.json"),
  ...
  test("uses process-start identity to distinguish a live owner from PID reuse", async () => {
    ...
    expect(
      await pathExistsForTest(
        join(reusedRoot, "packages/prompt-layer/assets/.mutation-lock"),
      ),
    ).toBe(false);
  });
  ...
  test("fails closed when process-start identity is unavailable", async () => {
    ...
    expect(
      await pathExistsForTest(
        join(acquisitionRoot, "packages/prompt-layer/assets/.mutation-lock"),
      ),
    ).toBe(false);
