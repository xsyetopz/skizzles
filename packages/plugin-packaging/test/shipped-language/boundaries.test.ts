...
import { SHIPPED_LANGUAGE_POLICY_PATHS } from "@skizzles/prompt-policy";
import {
...
describe("plugin shipped-language filesystem and resource boundaries", () => {
  test("rejects singular canonical symlinks before destination mutation", async () => {
    ...
    const source = join(root, "packages/command-routing/assets/hooks.json");
  ...
  test("rejects hardlinked and oversized canonical surfaces before destination mutation", async () => {
    ...
    const hardlinkSource = join(
      hardlinkRoot,
      "packages/command-routing/assets/hooks.json",
    );
  ...
  test("rejects noncanonical bytes at canonical and staged logo paths", async () => {
    ...
    await write(
      canonicalRoot,
      "packages/plugin-packaging/template/assets/logo.png",
      "I am sentient.\n",
    );
    await write(
      canonicalRoot,
    ...
    await copyFile(
      join(stagedRoot, "packages/plugin-packaging/template/assets/logo.png"),
