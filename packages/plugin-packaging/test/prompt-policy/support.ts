...
export async function coherentlyRewritePromptContract(
  ...
  await write(root, "packages/prompt-policy/assets/upstream/NOTICE", notice);
  manifest.upstream.notice = {
    path: "packages/prompt-policy/assets/upstream/NOTICE",
    ...noticeFact,
  };
  ...
  if (patchMode === "missing") {
    ...
    manifest.patch = {
      path: "packages/prompt-policy/assets/skizzles-base.patch",
      ...patchFact,
    };
