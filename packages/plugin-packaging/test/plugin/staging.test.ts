...
describe("plugin staging and discovery", () => {
  it("discovers default workspace paths independently of source depth", async () => {
    ...
    expect(paths).toEqual({
      repoRoot,
      templateRoot: join(repoRoot, "packages/plugin-packaging/template"),
  ...
  it("canonical hook discovery contract uses plugin-root commands", async () => {
    ...
    const hooks = await Bun.file(
      join(repoRoot, "packages/command-routing/assets/hooks.json"),
    ).json();
  ...
  it("stages only allowlisted canonical inputs deterministically", async () => {
    ...
    expect(
      await Bun.file(
        join(first, "packages/prompt-policy/assets/upstream/default.md"),
      ).exists(),
    ).toBe(false);
    expect(
      await Bun.file(
        join(first, "packages/prompt-policy/assets/skizzles-base.patch"),
      ).exists(),
    ).toBe(false);
    expect(
      await Bun.file(
        join(first, "packages/prompt-policy/assets/manifest.json"),
      ).exists(),
    ).toBe(false);
  ...
  it("rejects template-injected prompt baselines, patches, tooling, and transaction artifacts", async () => {
    for (const path of [
      ...
      "packages/prompt-policy/src/assets/manifest.ts",
      "packages/prompt-policy/test/nested/transaction.test.ts",
    ] as const) {
      const root = await fixture();
      await write(
        root,
        `packages/plugin-packaging/template/${path}`,
        "template-injected maintainer artifact\n",
      );
      const reportedPath = path.startsWith("packages/prompt-policy/assets/")
        ? "packages/prompt-policy/assets"
        : path.startsWith("packages/prompt-policy/src/")
    ...
    await mkdir(
      join(
        emptyDirectoryRoot,
        "packages/plugin-packaging/template/packages/prompt-policy/assets",
      ),
      { recursive: true },
    );
    await expect(
      ...
      new PackagingError(
        "Packaged plugin contains maintainer-only prompt-layer artifact packages/prompt-policy/assets.",
      ),
    );
  });
  ...
  it("rejects extra files in controlled prompt and OpenAI legal roots", async () => {
    for (const injection of [
      ...
      await write(
        root,
        `packages/plugin-packaging/template/${injection.path}`,
  ...
  it("preserves legitimate non-prompt template content", async () => {
    ...
    await write(
      root,
      "packages/plugin-packaging/template/docs/template-note.md",
