...
import { lockedDestinationError } from "../parent.ts";
import type { TransactionTarget } from "../path.ts";
import { transactionClaimPath } from "../path.ts";
import type { ClaimSnapshot } from "../transaction/claim.ts";
import { readClaim, removeClaim } from "../transaction/claim.ts";
import { UUID_PATTERN } from "../transaction/journal.ts";
import {
  claimRetirementConfirmed,
  hasValidRetirementMarker,
  removeRetirementArtifacts,
} from "../transaction/retirement.ts";
...
function parseRetirementMarkerTemp(
  ...
  if (!(GENERATION_PATTERN.test(generation) && UUID_PATTERN.test(token)))
    return;
  const parsed = Number(generation);
  return Number.isSafeInteger(parsed) && parsed <= MAX_RECOVERY_GENERATION
