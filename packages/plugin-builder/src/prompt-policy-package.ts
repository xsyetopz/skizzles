...
const PACKAGED_PROMPT_POLICY_DESCRIPTOR = "integrations/prompt-policy.json";
const PACKAGED_APPLIED_PROMPT = "instructions/skizzles-base.md";
const PACKAGED_PROMPT_PROVENANCE = "instructions/skizzles-base.provenance.json";
const PACKAGED_DEVELOPER_INSTRUCTIONS =
  "instructions/developer-instructions.md";
const PACKAGED_COMPACT_PROMPT = "instructions/compact-prompt.md";
...
const SOURCE_TO_PACKAGE_FILES = PROMPT_LAYER_PACKAGE_FILES;
...
function rejectMaintainerPromptArtifact(path: string): void {
  ...
  if (
    framedPath.includes(`/${PROMPT_LAYER_ASSET_ROOT}/`) ||
    framedPath.endsWith(`/${PROMPT_LAYER_ASSET_ROOT}`) ||
...
async function validatePromptPolicyArtifacts(
  ...
  const descriptorPath =
    mode === "source"
      ? PROMPT_LAYER_SOURCE_PATHS.descriptor
