...
const MANIFEST_PATH = "packages/core/prompt-layer/manifest.json";
const BASELINE_PATH = "packages/core/prompt-layer/upstream/default.md";
const LICENSE_PATH = "packages/core/prompt-layer/upstream/LICENSE";
const NOTICE_PATH = "packages/core/prompt-layer/upstream/NOTICE";
const PATCH_PATH = "packages/core/prompt-layer/skizzles-base.patch";
const OUTPUT_PATH = "instructions/skizzles-base.md";
const PROVENANCE_PATH = "instructions/skizzles-base.provenance.json";
const TRANSACTION_PATH = "packages/core/prompt-layer/.transaction";
const TRANSACTION_JOURNAL_PATH = `${TRANSACTION_PATH}/journal.json`;
const LOCK_PATH = "packages/core/prompt-layer/.mutation-lock";
...
async function reclaimStaleMutationLock(
  ...
  const quarantine = `packages/core/prompt-layer/.mutation-stale-${quarantineToken}`;
...
async function clearStaleReclaimClaim(
  ...
  try {
    ...
    claim = reclaimClaimValue(JSON.parse(bytes.toString("utf8")) as unknown);
  } catch (error) {
    const metadata = await lstat(absolute);
...
function reclaimClaimValue(value: unknown): ReclaimClaim {
  ...
  assertKeys(
    object,
    ["version", "pid", "processStartIdentity", "token", "createdAtUnixMs"],
...
async function releaseMutationLock(
  ...
  const releasePath = `packages/core/prompt-layer/.mutation-release-${lock.owner.token}`;
...
async function processOwnerState(
  ...
  if (!processExists(owner.pid)) return "stale";
  ...
  if (!validProcessStartIdentity(actual)) return "unknown";
...
async function readOwnerFile(
  ...
  validateText(bytes, "prompt mutation lock owner");
  try {
    return lockOwnerValue(JSON.parse(bytes.toString("utf8")) as unknown);
  } catch (error) {
    throw new PromptLayerError(
      `Prompt mutation lock owner is invalid: ${errorMessage(error)}`,
    );
...
async function removeOwnedLockDirectory(
  ...
  const quarantine = `packages/core/prompt-layer/.mutation-release-${token}`;
  if (await pathExists(join(root, quarantine))) return;
  await assertContainedPath(root, quarantine, false);
  await assertFilesystemIdentity(
    absolute,
    identity,
...
async function cleanupMutationOrphans(
  ...
  const layerPath = join(root, "packages/core/prompt-layer");
...
function mutationOrphanKind(name: string): "stale" | "release" | undefined {
  if (name.startsWith(".mutation-stale-")) return "stale";
  if (name.startsWith(".mutation-release-")) return "release";
  return undefined;
}
...
async function cleanupMutationOrphan(
  ...
  const relativePath = `packages/core/prompt-layer/${entry.name}`;
  await assertContainedPath(root, relativePath, true);
  const absolute = join(root, relativePath);
  const metadata = await lstat(absolute);
...
function throwInjectedPromotionFault(
  ...
  if (fault?.promotionIndex !== index) return;
...
async function rollbackPreparedTransaction(
  ...
  for (const [index, entry] of journal.entries.entries()) {
    if (states[index] === "new") {
      await assertContainedPath(root, entry.path, true);
      await writeAtomically(join(root, entry.path), backups[index] as Buffer);
    }
  }
...
async function runCli(): Promise<void> {
  ...
  switch (command) {
    case "build":
      if (args.length > 0) throw new PromptLayerError("Usage: prompt:build");
      await buildPrompt(root);
      break;
    case "check":
      if (args.length > 0) throw new PromptLayerError("Usage: prompt:check");
    ...
    default:
      throw new PromptLayerError(
        "Usage: prompt-layer.ts <build|check|patch|rebase>",
      );
  }
}
