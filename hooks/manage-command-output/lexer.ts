...
function consumeSeparator(
  ...
  if (state.words.length === 0 && !state.wordStarted) return undefined;
  if (separator === ";" && script[index + 1] === ";") return undefined;
  if (
    (separator === "&" || separator === "|") &&
  ...
  if (!finishCommand(state)) return undefined;
...
export function simpleCommands(script: string): SimpleCommand[] | undefined {
  ...
  for (let index = 0; index < script.length; index += 1) {
    ...
    if (isUnsupportedSyntax(character)) return undefined;
    if (character === "#" && state.atWordStart) return undefined;
    const nextIndex = consumeCharacter(state, script, index, character);
    if (nextIndex === undefined) return undefined;
    index = nextIndex;
  }

  if (state.requiresCommand || !finishCommand(state)) return undefined;
