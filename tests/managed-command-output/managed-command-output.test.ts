...
function exactStatus(content: string): Record<string, unknown> {
  if (content.length === 0) throw new Error("status snapshot was empty");
  ...
  if (!validKeys) throw new Error(`unexpected status keys: ${keys.join(",")}`);
  if (typeof status["id"] !== "string") throw new Error("status id missing");
...
describe("managed command output runner", () => {
  ...
  test("retention cannot delete an active concurrent run", async () => {
    ...
    if (exitCode === undefined) stopProcess(active.pid);
  ...
  test("serves exact status, tail, errors, and search queries from retained artifacts", () => {
    ...
    if (!id) throw new Error(`run id missing from ${directory}`);
