...
- **Status:** Accepted
- **Date:** 2026-07-19
- **Decision owner:** `@skizzles/workspace-policy`
...
```sh
bun test packages/workspace-policy/test/workspace/source-imports.test.ts
bun run --cwd packages/workspace-policy typecheck
bun run --cwd packages/workspace-policy check
bun run --cwd packages/workspace-policy build
bun packages/workspace-policy/dist/cli.js .
bun -e 'import("./packages/workspace-policy/dist/workspace/policy.js")'
...
syntax. The workspace-policy owner owns API/build behavior; release reviewers own the
