...
- **Status:** Accepted
- **Date:** 2026-07-19
- **Decision owner:** `@skizzles/scratchspace`
...
Create the zero-workspace-dependency package `@skizzles/scratchspace`. One
...
- bounded command-observation output retained as operational evidence;
...
- **Put the manager in command-observation:** rejected because policy, prompt, model,
...
scratchspace paths, and direct recursive disposal aliases, but deliberately does not
...
```sh
bun run --cwd packages/scratchspace check
bun run --cwd packages/scratchspace typecheck
bun run --cwd packages/scratchspace test
bun run --cwd packages/scratchspace build
...
hard-coded host temporary paths, and direct recursive disposal of scratchspace nested
