...
`packages/usage-analysis/test/privacy-boundary.test.ts`, which requires one exact
...
the commits. The workspace-governance gate rejects mutable, unknown, mismatched, missing,
...
`@skizzles/workspace-governance` owns parsing, acquisition, extraction, execution, and
...
```sh
bun test packages/workspace-governance/test/repository-security/tools.test.ts
...
bunx @biomejs/biome@2.5.4 check --config-path biome.jsonc --vcs-root . \
  ...
  packages/workspace-governance/src/repository-security \
  packages/workspace-governance/src/security-cli.ts \
  packages/workspace-governance/test/repository-security/tools.test.ts
