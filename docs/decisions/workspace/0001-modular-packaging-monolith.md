...
declared internal package dependency: `@skizzles/plugin-packaging` depends on
`@skizzles/prompt-policy`. The root orchestrates package-owned commands; it is not a
...
6. `packages/plugin-packaging/` is the only plugin staging composition root.
   `packages/prompt-policy/` owns prompt inputs and provenance. Canonical packages and
