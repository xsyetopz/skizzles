...
- Keep `environment` as explicit command-service list-form forwarding. Use `compose_environment` as the only authorization for project-source interpolation and valueless service-environment/build-argument host reads. Treat each `secret_environment` name as a required project-owned top-level secret source `{ environment: VAR }`; reject its overlap with both other fields and pass secret values only to Compose up.
- Validate source/base Compose files before override generation with non-interpolated, non-normalized, no-env-resolution JSON. Reject undeclared braced, unbraced, default, alternative, required, or nested reads, implicit reads, plaintext secret use, service `env_file`, project `.env`, and top-level `configs.environment`; preserve `$$` literals.
- Persist the exact validated raw JSON as the sole Compose source for normalized validation, up, status, logs, attached execution, and termination. Use explicit `/dev/null` interpolation input so project `.env` cannot re-enter later, and require recreation for legacy runtime state without a materialized source while retaining exact-label destruction.
- Construct every Docker subprocess environment from one exact client allowlist plus only present manifest-authorized non-secret names. Never inherit `COMPOSE_*` topology controls or ambient names by prefix. Thread the service environment through all lifecycle paths.
- Sanitize every local Git process to path/temp/fixed locale, no system/global configuration, no prompt, and command-scoped disabled hooks and filesystem monitors. Never inherit ambient repository/index/object, credential/askpass/SSH, trace, pager, or editor controls; clone local repositories only.
