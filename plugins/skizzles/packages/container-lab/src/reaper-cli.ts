...
function spawnCommandGuardian(command, args, options) {
  ...
  const child = Bun.spawn([process.execPath, "--eval", GUARDIAN_SOURCE, specification], {
    detached: true,
...
function signalOwnedGuardian(guardianExited, pid, signal, kill = process.kill) {
  if (guardianExited)
    return false;
  try {
    kill(-pid, signal);
...
async function runCommand(command, args, options = {}) {
  assertProcessPlatform(process2.platform, command);
  if (options.signal?.aborted) {
    throw new Error(`${command} aborted`);
  }
...
async function readTrustedUnknownJson(root, parentSegments, fileName, label, options = {}) {
  ...
  if (!before) {
    let unexpected;
    try {
      unexpected = await open(candidate, constants.O_RDONLY | constants.O_NOFOLLOW);
    } catch (error) {
      if (error.code === "ELOOP") {
...
async function exactDirectoryChainIdentity(root, segments, label, options) {
  ...
  for (const segment of segments) {
    ...
    const identity2 = await exactDirectory(candidate, expected, label, options);
    if (!identity2) {
      return;
    }
    identities.push({ path: candidate, ...identity2 });
  }
  return identities;
}
...
async function withFileLock(path2, operation, options = {}) {
  ...
  for (let attempt = 0;attempt < attempts; attempt++) {
    ...
    const candidate = `${path2}.candidate-${process3.pid}-${crypto.randomUUID()}`;
    let acquired = false;
    try {
...
async function removeConfirmedStaleLock(path2, staleMs, processProbe) {
  ...
  try {
    ...
    try {
      ...
      const value = JSON.parse(contents);
      if (isRecord3(value) && typeof value["pid"] === "number" && Number.isInteger(value["pid"]) && value["pid"] > 0 && typeof value["createdAt"] === "string") {
        record = value;
      }
    } catch {}
    if (record === undefined) {
...
async function reclaimSameLock(path2, inspected, staleMs, processProbe) {
  const candidate = `${path2}.reclaim-candidate-${process3.pid}-${crypto.randomUUID()}`;
  try {
    await writeFile2(candidate, JSON.stringify({
      pid: process3.pid,
      createdAt: new Date().toISOString()
...
function assertLocalClone(args) {
  ...
  if (!args.includes("--local") || typeof source !== "string" || typeof destination !== "string" || !isAbsolute2(source) || !isAbsolute2(destination)) {
...
function resolveRoots(options = {}) {
  return {
    stateRoot: resolve2(options.stateRoot ?? process4.env["CODEX_CONTAINER_LAB_STATE_ROOT"] ?? defaultStateRoot()),
    runtimeRoot: resolve2(options.runtimeRoot ?? process4.env["CODEX_CONTAINER_LAB_RUNTIME_ROOT"] ?? defaultRuntimeRoot())
  };
}
function resolveOwner(explicit, environment = process4.env) {
...
function labLockPath(stateRoot, owner, labId) {
  ...
  return join2(ownerDirectory(stateRoot, owner), ".locks", `lab-${labId}`);
}
function activityLockPath(stateRoot, owner, labId) {
  safeStateName(labId, "lab id");
  return join2(ownerDirectory(stateRoot, owner), ".locks", `activity-${labId}`);
}
...
function assertLabMetadata(value, roots, owner, labId) {
  try {
    safeStateName(labId, "lab id");
    resolveOwner(owner, {});
    if (!isRecord4(value) || value["version"] !== 1 || value["id"] !== labId || value["owner"] !== owner || value["ownerKey"] !== ownerKey(owner)) {
...
function validatePersistedRuntime(lab, runtime) {
  ...
  const expectedSource = join3(runtimeRoot, "source.compose.json");
  const hasImmutableSource = runtime["sourceFile"] !== undefined;
  const expectedBase = hasImmutableSource || mode.kind === "compose" ? undefined : join3(runtimeRoot, "base.compose.yaml");
...
function validatedPersistedConfig(lab, config) {
  ...
  if (!(isNormalizedAbsolute(sourceRoot) && isNormalizedAbsolute(manifestPath)) || config["repoRoot"] !== sourceRoot || config["manifestPath"] !== manifestPath || !hasOnlyKeys(config, [
    ...
    "composeEnvironment",
    "secretEnvironment"
  ]) || !isRecord4(config["mode"]) || !isRecord4(config["runtime"])) {
...
function normalizeEnvironmentMetadata(lab) {
  ...
  if (isRecord4(lab["runtime"]) && isRecord4(lab["runtime"]["config"])) {
...
function isEndpoint(value) {
  return isRecord4(value) && typeof value["name"] === "string" && SERVICE_NAME.test(value["name"]) && typeof value["service"] === "string" && SERVICE_NAME.test(value["service"]) && typeof value["target"] === "number" && Number.isInteger(value["target"]) && value["target"] >= 1 && value["target"] <= 65535 && isBoundedString(value["url"], 2048);
}
function isDeclaredPort(value) {
  return isRecord4(value) && hasOnlyKeys(value, ["name", "service", "target", "scheme"]) && typeof value["name"] === "string" && SERVICE_NAME.test(value["name"]) && typeof value["service"] === "string" && SERVICE_NAME.test(value["service"]) && typeof value["target"] === "number" && Number.isInteger(value["target"]) && value["target"] >= 1 && value["target"] <= 65535 && (value["scheme"] === undefined || typeof value["scheme"] === "string" && URL_SCHEME.test(value["scheme"]));
}
...
function isFinding(value) {
  return isRecord4(value) && (value["service"] === undefined || isBoundedString(value["service"], 128)) && typeof value["surface"] === "string" && FINDING_SURFACES.has(value["surface"]) && isBoundedString(value["detail"], 1024);
}
...
async function retirementAncestorState(targetRoot, relative3, journalOwnedPaths) {
  const canonical = await canonicalRoot(targetRoot);
  const parts = relative3.split("/").slice(0, -1);
...
function createdDirectoryConflict(relative3) {
  return new Error(`recovery conflict at ${relative3}; divergent target directory preserved`);
}
...
async function readOwnerManifest(path7) {
  const resolvedPath = resolve5(path7);
  ...
  if (basename(resolvedPath) !== "owner.json" || basename(owners) !== "owners") {
    throw new Error(`invalid owner manifest path: ${path7}`);
  }
  ...
  if (!isRecord5(value) || value["version"] !== 1 || typeof value["owner"] !== "string" || typeof value["ownerKey"] !== "string" || !isTimestamp2(value["createdAt"])) {
    throw new Error(`invalid owner manifest: ${path7}`);
  }
  resolveOwner(value["owner"], {});
  if (value["ownerKey"] !== ownerKey(value["owner"]) || basename(resolve5(path7, "..")) !== value["ownerKey"]) {
    throw new Error(`owner manifest hash mismatch: ${path7}`);
  }
  return {
    version: 1,
    owner: value["owner"],
...
async function reapArchivedOwners(options) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      let initial;
      try {
        initial = (options.stateReader ?? queryThreadState)(database, owner.owner);
      } catch (error) {
        return {
          ok: false,
          archivedOwnersCleaned: [],
        ...
        await withFileLock(ownerLockPath(roots.stateRoot, owner.owner), async () => {
          ...
          for (const lab of labs) {
            await prepareExactLab(roots, lab, async (claimed) => {
              await cleanupExactLab(roots, claimed, options.docker ?? defaultDockerRunner, options.environment ?? {}, async () => {
                await options.beforeRecheck?.(owner.ownerKey);
                let rechecked;
                try {
                  rechecked = (options.stateReader ?? queryThreadState)(database, owner.owner);
                } catch {
          ...
          try {
            ...
            throw new Error("thread row could not be rechecked before final cleanup");
          }
  ...
  result.archivedOwnersCleaned = result.archivedOwnersCleaned.slice(0, 1e4);
  result.retainedOwners = result.retainedOwners.slice(0, 1e4);
...
function queryThreadState(database, owner) {
  ...
  if (archived === 1 && row.archived_at !== null && (typeof row.archived_at === "bigint" || Number.isInteger(row.archived_at))) {
    return "archived";
  }
  return "uncertain";
}
...
async function cleanupExactLab(roots, lab, docker, environment, authorize) {
  ...
  await withFileLock(activityLock, async () => await withFileLock(labLock, async () => {
    ...
    await assertOwnerStateDirectory(roots.stateRoot, lab.ownerKey, "owner state directory disappeared");
...
async function boundedRemove(root, maxEntries) {
  ...
  async function scan(path7) {
    ...
    for (const name of await readdir3(path7)) {
      if (++count > maxEntries) {
        throw new Error("cleanup path exceeds bounded entry limit");
      }
      await scan(join7(path7, name));
    }
  }
  await scan(root);
...
async function reaperMain(args = process5.argv.slice(2)) {
  try {
    ...
    if (parsed.version) {
      writeOutput({ version: CONTAINER_LAB_VERSION });
      return 0;
    }
...
function reaperOutput(result) {
  const exceptionalRetentions = result.retainedOwners.filter((item) => item.reason !== "thread is active");
  if (result.ok && result.archivedOwnersCleaned.length === 0 && result.errors.length === 0 && exceptionalRetentions.length === 0) {
    return;
  }
...
function writeOutput(value, stream = process5.stdout) {
...
function distinctBounded(items) {
  const details = [];
...
function boundedRedacted(value, maxBytes) {
...
if (import.meta.main) {
  process5.exit(await reaperMain());
}
export {
  reaperOutput,
  reaperMain,
