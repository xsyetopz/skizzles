...
function classify(source: unknown): Actor {
  ...
  if (property(subagent, "other") === "guardian") return "guardian";
...
function readSessionMeta(session: SessionSummary, payload: unknown): void {
  ...
  if (agentPath !== undefined) session.agentPath = agentPath;
}
...
function readTurnContext(
  ...
  if (!state.reachedOwnTurn) return;
  state.currentModel =
...
function readTokenCount(
  ...
  if (!state.reachedOwnTurn) {
    state.previousTotal = total ?? state.previousTotal;
    if (signature !== undefined) state.previousSignature = signature;
    return;
  }
  if (signature && signature === state.previousSignature) return;
...
function readGuardianReview(session: SessionSummary, payload: unknown): void {
  ...
  try {
    const assessment = JSON.parse(message) as unknown;
    const outcome = property(assessment, "outcome");
    if (outcome === "allow") session.reviewAllow += 1;
    if (outcome === "deny") session.reviewDeny += 1;
  } catch {
...
export async function parseRollout(
  ...
  for await (const event of readEvents(path)) {
    ...
    if (eventType !== "event_msg") continue;
    const payloadType = property(payload, "type");
    ...
    if (
      ...
      if (payloadType === "token_count") updatePreviousUsage(state, payload);
      continue;
    }
