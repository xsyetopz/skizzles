...
function isContainerLabRun(command: SimpleCommand): boolean {
  ...
  while (containerLabGlobalOptions.has(words[index] ?? "")) {
    if (!hasContainerLabOptionValue(command, index)) return false;
    index += 2;
  }
...
function isKnownNormalizedCommand(words: string[]): boolean {
  ...
  if (program === "flutter") return flutterActions.has(subcommand ?? "");
  if (program === "dart") return dartActions.has(subcommand ?? "");
  if (program === "cargo") return isCargoCommand(words);
  if (program === "xcodebuild") return true;
  if (program === "swift") return swiftActions.has(subcommand ?? "");
  return (
...
export function isRecognized(command: SimpleCommand | undefined): boolean {
  ...
  if (isContainerLabRun(command)) return true;
