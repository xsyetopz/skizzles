...
function consumeXcrunSdk(cursor: Cursor): boolean {
  ...
  if (!(value && xcrunSdks.has(value) && isCertain(cursor, 1))) return false;
  cursor.index += 2;
  return true;
}
...
function consumeXcrunOptions(cursor: Cursor): boolean {
  while (true) {
    ...
    if (!(option && isCertain(cursor))) return false;
    if (option === "--") {
      cursor.index += 1;
      return true;
    }
    if (xcrunSdkOptions.has(option)) {
      if (!consumeXcrunSdk(cursor)) return false;
      continue;
    }
...
function consumeLauncher(cursor: Cursor): boolean {
  ...
  if (launcher !== "rustup" || !isCertain(cursor)) return true;
...
export function normalizeCommand(
  ...
  if (!first || assignment.test(first) || first === "env") return undefined;
  if (!consumeLauncher(cursor)) return undefined;

  const program = current(cursor);
  if (!(isCanonicalWord(program) && isCertain(cursor))) return undefined;
  return {
